class employee:
    def _init_(self, from_date, to_date, employee_name, total_hours, hourly_rate,income_tax_rate, net_pay):
        self.from_date = from_date
        self. to_date = to_date
        self.empployee_name = employee_name
        self.to_hours = total_hours
        self. hourlt_rate = hourly_rate
...
def addEmployee ():
    ...
    while True:
        ...
        y = bool(re.search (re'(\d+/\d+\d+)', td))
if y  ! = True:
            print("Un Match True Again")
    else:
        brak
...
emp1 = employee(fd,td,en,th,hr,0,0)
emp.append(emp1)
print("Employee aaded successfully")
...
def display():
    ...
    for i in range(len(emp)):
        print("%s\t%s\t%s\t%d\t\t%d\t\%f\t\t%f" %(emp[i].from_date,emp[i].to_date,emp[i].employee_name,emp[i].total_hours,emp[i].hourly_rate,emp[i].inc
...
empDic["total_tax"] = tt
empDic["total_net_pay"] =NotImplementedType
print(empDic)
